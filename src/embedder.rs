...
impl Embedder {
    /// Create a new Embedder using the default model URL.
    pub fn new() -> Result<Self> {
        Self::new_with_url(None)
    }

    /// Create a new Embedder but allow overriding the base URL used to download the model.
    ///
    /// * `model_url_base` – If `Some(url)`, that URL will be used instead of the internal
    ///   default.  It must point to a directory containing the same filenames as
    ///   `MODEL_FILES` (e.g. `model.safetensors`, `tokenizer.json`).  If `None`, the built-in
    ///   default URL is used.
    pub fn new_with_url(model_url_base: Option<&str>) -> Result<Self> {
        ...
        let base_url = model_url_base.unwrap_or(MODEL_URL_BASE);
        let model_path = Self::ensure_model_files(base_url)?;
        // Load tokenizer
    ...
    /// models are expected to be in the models/static-retrieval-mrl-en-v1 directory
    fn ensure_model_files(base_url: &str) -> Result<PathBuf> {
        ...
        for file in MODEL_FILES.iter() {
            ...
            if !file_path.exists() {
                ...
                // directory is basically created earlier in the function
                // so this is just done defensively
    ...
    /// Main embedding function. Embed a batch of strings and return as Tensor
...
mod tests {
    ...
    fn test_embed_batch_vec_structure() -> Result<()> {
        ...
        tracing::info!("Initializing embedder for test...");
        let embedder = Embedder::new()?;
        let expected_dim = embedder.embedding_dim;
        tracing::info!("Embedder initialized. Expected dimension: {}", expected_dim);

        // 2. Define simple test input
        let texts = vec!["Test sentence one.", "Test sentence two."];
        tracing::info!("Embedding test texts...");
    ...
    fn test_embed_batch_vec_semantic_similarity() -> Result<()> {
        ...
        tracing::debug!("Similarity (dog vs canine): {:.4}", sim_dog_canine);
        tracing::debug!(
            "Similarity (quantum vs physics): {:.4}",
            sim_quantum_physics
        );
        tracing::debug!("Similarity (dog vs quantum): {:.4}", sim_dog_quantum);
        tracing::debug!("Similarity (canine vs physics): {:.4}", sim_canine_physics);
        tracing::debug!("Similarity (dog vs mountain): {:.4}", sim_dog_mountain);
        tracing::debug!(
        ...
        tracing::info!("Dog ratio (similar/dissimilar): {:.2}", dog_ratio);
        tracing::info!("Physics ratio (similar/dissimilar): {:.2}", physics_ratio);
        ...
        tracing::info!("Semantic similarity verified with relative comparisons.");
